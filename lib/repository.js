...
const get = co.wrap(function * (repoUrl) {
  var lastCommit;
  switch (urlParser.parse(repoUrl).host) {
    case 'github.com':
    case 'www.github.com':
      lastCommit = yield github(repoUrl);
      break;
    default:
      lastCommit = yield gitlab(repoUrl);
  }

  return {
    url: {
      complete: repoUrl,
      host: urlParser.parse(repoUrl).host,
      path: urlParser.parse(repoUrl).pathname
    },
    lastCommit
  };
});
